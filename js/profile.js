window.renderProfile = async function() {
    ...
    try {
        ...
        if (profileRes.error) {
            console.error("Profile 讀取失敗詳細原因:", profileRes.error);
        ...
        if (myPosts.length === 0) {
            html += `<div class="flex flex-col items-center justify-center py-20 px-4 text-center">
            ...
            html += `<div class="masonry-grid px-2">` + myPosts.map(post => `
        ...
        container.innerHTML = `<div class="p-10 text-center text-red-500 mt-20 flex flex-col items-center"><i class="fa-solid fa-triangle-exclamation text-3xl mb-3"></i>讀取失敗，請確認網路或資料庫結構。<br><span class="text-xs text-gray-400 mt-2">提示：若剛修改過資料表結構，請嘗試登出並重新註冊帳號。</span></div>`;
    }
}
...
window.renderSubsList = function() {
    ...
    if (subs.length === 0) container.innerHTML = `<p class="text-xs text-gray-400 text-center py-4 bg-white rounded-xl border border-gray-100">目前沒有追蹤任何人</p>`;
    else container.innerHTML = subs.map(sub => `<div class="flex items-center gap-3 bg-white p-3 rounded-xl shadow-sm cursor-pointer border border-gray-50" onclick="window.location.href='profile.html?userId=${sub.id}'"><img src="${sub.avatar}" class="w-10 h-10 rounded-full object-cover"><span class="font-bold text-sm text-gray-800 flex-1">${sub.name}</span><i class="fa-solid fa-chevron-right text-gray-300 text-xs"></i></div>`).join('');
}
window.renderFansList = function() {
    const container = document.getElementById('fans-list');
    if (!container) return;
    const fans = getFans();
    if (fans.length === 0) container.innerHTML = `<p class="text-xs text-gray-400 text-center py-4 bg-white rounded-xl border border-gray-100">目前還沒有粉絲</p>`;
    else container.innerHTML = fans.map(fan => `<div class="flex items-center gap-3 bg-white p-3 rounded-xl shadow-sm border border-gray-50"><img src="${fan.avatar}" class="w-10 h-10 rounded-full object-cover"><span class="font-bold text-sm text-gray-800 flex-1">${fan.name}</span></div>`).join('');
}
