...
describe('switchFansTab Error Handling', () => {
    ...
    test('handles Supabase fetch error correctly for fans tab', async () => {
        ...
        const eqSpy = jest.fn().mockResolvedValue({ data: null, error: mockError });
        ...
        expect(eqSpy).toHaveBeenCalledWith('creator_id', 'user-1');
    ...
    test('handles Supabase fetch error correctly for subs tab', async () => {
        ...
        expect(fromSpy).toHaveBeenCalledWith('subscriptions');
        expect(selectSpy).toHaveBeenCalledWith('*');
        expect(eqSpy).toHaveBeenCalledWith('subscriber_id', 'user-1');
